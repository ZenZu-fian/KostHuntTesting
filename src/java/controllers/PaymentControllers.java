...
public class PaymentControllers extends HttpServlet {
     ...
     */
    // Ganti method showPaymentForm di PaymentControllers.java dengan ini:
    private void showPaymentForm(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.err.println("SQL Error in showPaymentForm: " + e.getMessage());
            e.printStackTrace();
    ...
    /**
     * Create payment and update room status FR-07: Mencatat Pembayaran FR-15:
     * Mendaftar Kos
     */
    // Ganti method createPayment di PaymentControllers.java dengan ini:
    private void createPayment(HttpServletRequest request, HttpServletResponse response)
        ...
        String tenantName = (String) session.getAttribute("userName");
        ...
        try {
            ...
            // 3. HAPUS PENGECEKAN ROOM STATUS - BIAR BISA BAYAR LAGI
            // Tenant yang sudah menyewa harus bisa bayar untuk bulan berikutnya
            ...
            // 5. Update room status HANYA jika masih Available (pembayaran pertama)
            String updateRoomQuery = "UPDATE rooms SET status = 'Occupied' WHERE idRoom = ? AND status = 'Available'";

            try (PreparedStatement stmt = db.getConnection().prepareStatement(updateRoomQuery)) {
                stmt.setInt(1, roomId);
                int rowsUpdated = stmt.executeUpdate();
                System.out.println("Room status updated: " + rowsUpdated + " rows");
            }

            // 6. Update tenant room_id jika belum ada
            String updateTenantQuery = "UPDATE tenants SET room_id = ? WHERE idTenant = ? AND (room_id IS NULL OR room_id = 0)";
            try (PreparedStatement stmt = db.getConnection().prepareStatement(updateTenantQuery)) {
                stmt.setInt(1, roomId);
                stmt.setInt(2, tenantId);
                int rowsUpdated = stmt.executeUpdate();
                System.out.println("Tenant room_id updated: " + rowsUpdated + " rows");
            }

            db.getConnection().commit();

            // Success - redirect to tenant dashboard with success message
            session.setAttribute("successMessage", "Pembayaran berhasil! Terima kasih.");
            response.sendRedirect("tenantDashboard");
            ...
            // Redirect back to payment form with error
            session.setAttribute("errorMessage", "Pembayaran gagal: " + e.getMessage());
            response.sendRedirect("payment?action=showForm&roomId=" + roomId + "&kostId=" + kostId);
    ...
    /**
     * Show payment history for tenant FR-08: Histori Pembayaran
     */
